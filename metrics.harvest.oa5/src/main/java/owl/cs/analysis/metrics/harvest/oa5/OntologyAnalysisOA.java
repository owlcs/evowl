...
import owl.cs.analysis.metrics.utilities.oa5.StaticMetrics;
...
public class OntologyAnalysisOA extends OntologyAnalysis {
	...
	protected boolean owlapiParse() {
		try {
			...
			addResult(MetricLabels.OA5_PARSEABLE, true);
			return true;
		} catch (OWLOntologyCreationException e) {
			e.printStackTrace();
			addResult(MetricLabels.OA5_PARSEABLE, false);
			addResult(MetricLabels.OA5_LOAD_EXCEPTION_MESSAGE, e.getMessage());
		}
		return false;
	}
